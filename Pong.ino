...
void showScore()
{
  ...
  {
    setLedColor(PLAYER1, LEDS_NB / 2 - (i + 1), PG_PLAYER1_COLOR);
  }

  // On allume le nombre de led correspondant au score du player 2
  for (int i = 0; i < player2Score; i++)
  {
    setLedColor(PLAYER2, LEDS_NB / 2 - (i + 1), PG_PLAYER2_COLOR);
  }
  ...
  if (lastWinner == PLAYER1)
  {
    for (int i = 0; i < 3; i++)
    {
      ...
      delay(500);
      setLedColor(PLAYER1, LEDS_NB / 2 - player1Score, NOIR);
      FastLED.show();

      // On allume la derniere LED pendant 0.5s
      delay(500);
      setLedColor(PLAYER1, LEDS_NB / 2 - player1Score, PG_PLAYER1_COLOR);
      FastLED.show();
    }
  }
  else // lastWinner == PLAYER2
  {
    for (int i = 0; i < 3; i++)
    {
      // On eteint la derniere LED pendant 0.5s
      delay(500);
      setLedColor(PLAYER2, LEDS_NB / 2 - player2Score, NOIR);
      FastLED.show();

      // On allume la derniere LED pendant 0.5s
      delay(500);
      setLedColor(PLAYER2, LEDS_NB / 2 - player2Score, PG_PLAYER2_COLOR);
      FastLED.show();
    }
  }
...
void pongGameLoop()
{
  ...
  for (int i = 0; i < PG_HIT_ZONE; i++)
  {
    ...
    setLedColor(PLAYER1, i, PG_PLAYER1_COLOR / 10);  // On divise la couleur par 10 pour la rendre 10 fois moins puissante
    setLedColor(PLAYER2, i, PG_PLAYER2_COLOR / 10);
  }
  ...
  for (int i = 0; i < player1Score; i++)
  {
    setLedColor(PLAYER1, LEDS_NB / 2 - (i + 1), PG_PLAYER1_COLOR / 15);
  }
  // Pour le player 2
  for (int i = 0; i < player2Score; i++)
  {
    setLedColor(PLAYER2, LEDS_NB / 2 - (i + 1), PG_PLAYER2_COLOR / 15);
  }
